// app.js — NombreMot : le compte est bon
//
// Jeu "compte est bon" dont les jetons sont les nombres-mots-simples qui
// composent l'écriture en toutes lettres de la cible (voir Mots.js). Deux
// façons de répondre, au choix (bascule "Réponse" dans le panneau latéral) :
//   - "expression" : on tape une expression avec les jetons en CHIFFRES
//     (comme le compte-est-bon classique), + − × ÷ et parenthèses.
//   - "arbre" : on construit visuellement l'arbre de calcul, feuilles en
//     LETTRES uniquement, avec + et × seulement (voir nombreMotArbre).
...
// Mode "expression" : les 4 opérations classiques (texte libre, comme le
// compte-est-bon d'origine). Mode "arbre" : uniquement + et × — la
// décomposition d'un nombre-mot (Mots.js/getNombre) ne s'exprime jamais
// autrement (ex : quatre-vingt-seize = 4×20+16), donc construire un arbre
// avec − ou ÷ n'aurait pas de sens pédagogique.
...
// Affichage de la cible : les deux bascules sont indépendantes (non
// exclusives) — on peut afficher les chiffres, le mot, ou les deux à la fois,
// mais pas aucun des deux.
let afficherChiffres = true;
...
// ==================== ARBRE DE CALCUL (réduction + LaTeX) — partagé ====================
// Représentation commune aux deux modes de réponse une fois la saisie
// interprétée : {type:'NUM',nombre} / {type:'BINOP',op,gauche,droite}.
// Le mode expression l'obtient en parsant le texte (plus bas) ; le mode
// arbre en repliant son arbre visuel n-aire en chaîne binaire gauche-
// associative (voir construireArbreCalcul).
...
// ==================== GÉNÉRATION (partagée) ====================
// Les jetons sont proposés dans leur ordre d'apparition dans le mot (celui
// que renvoie tirerNombreParJetons), sans les mélanger : comme la cible est
// maintenant tirée parmi toute une famille de mots réalisant ces jetons
// (voir Mots.js), cet ordre varie déjà naturellement d'un tirage à l'autre
// et ne trahit plus systématiquement la structure de la décomposition.
function genererQuestion() {
  const nbJetons = MODES[modeActuel].jetons;
  const d = tirerNombreParJetons(nbJetons);
  if (!d) throw new Error("Aucun mot trouvé pour ce mode (" + nbJetons + " jetons)");
  return d;
}
...
// les jetons cliquables du mode expression, dont le texte doit rester des
// chiffres bruts pour correspondre exactement à ce qui est inséré/comparé
// dans le champ de saisie (voir extractNumbers/checkJetons).
function formaterMilliers(n, separateur = " ") {
  return String(n).replace(/\B(?=(\d{3})+(?!\d))/g, separateur);
}

function rendreConsigne() {
  const chiffres = formaterMilliers(game.n);
  const mot = game.mot;

  if (afficherMot && afficherChiffres) {
    consigne.innerHTML = `<span class="consigne-mot">${mot}</span><div class="consigne-valeur">${chiffres}</div>`;
  } else if (afficherMot) {
    consigne.innerHTML = `<div class="consigne-valeur">${mot}</div>`;
  } else {
    consigne.innerHTML = `<div class="consigne-valeur">${chiffres}</div>`;
  }
}
...
function tableauComparatifVide() {
  return `
    <table class="tableau-comparatif">
      <thead><tr><th>Ton calcul</th><th>Cible</th></tr></thead>
...
// ==================== RÉSERVE DE JETONS ====================
// Mode expression : jetons EN CHIFFRES, cliquables (insèrent dans le champ).
// Mode arbre : jetons EN LETTRES, lecture seule (le choix se fait dans le
// popup d'une case de l'arbre) — leur état "utilisé" suit jetonsUtilises.
...
// ==================== MODE EXPRESSION — INSERTION JETONS / OPÉRATEURS ====================
...
// varie avec la longueur du mot affiché — px/py représentent donc le CENTRE
// horizontal de chaque noeud, jamais son bord gauche (voir layoutArbre et
// le CSS transform:translateX(-50%) associé à .arbre-noeud).
...
function libelleNoeud(noeud) {
  ...
  if (noeud.type === "jeton") return getMot(game.jetons[noeud.jetonIndex]);
...
function construireMenuPlaceholder(noeud) {
  ...
  if (dispo.length > 0) {
    dispo.forEach(idx => {
      ...
      b.textContent = getMot(game.jetons[idx]);
...
// En quiz : abandon volontaire, révèle la décomposition d'origine, compte
// comme une réponse fausse (sans pénalité), puis attend "Question suivante".
...
function validerExpression() {
  ...
  finQuestion();
  document.querySelectorAll(".jeton").forEach(j => j.classList.add("vert"));
  definirMessage("Ton expression donne un résultat différent de la cible.", "erreur");
  afficherVerification(tableau);
  majBandeau();
  MathJax.typesetPromise([message, zoneVerificationContenu]);
}
...
function validerArbre() {
  ...
  finQuestion();
  definirMessage("Ton arbre donne un résultat différent de la cible.", "erreur");
  afficherVerification(tableau);
  majBandeau();
  MathJax.typesetPromise([message, zoneVerificationContenu]);
}
...
// dForce : décomposition déjà prête (voir choisirNombrePersonnalise) à
// utiliser telle quelle au lieu de tirer un nombre au hasard — pour le
// mode "choisir son nombre" (utile en classe pour cibler un nombre précis).
function nouvelleQuestion(dForce) {
  if (!dForce && etatJeu === "quiz" && questionIndex >= NB_QUESTIONS) {
    afficherFin();
    return;
  }
  ...
  const d = dForce || genererQuestion();
  game = {
    n: d.n,
    mot: d.mot,
    jetons: d.jetons,
...
function construireBoutonsAffichage() {
  ...
  conteneur.className = "param-buttons";
  const options = [
    { id: "chiffres", label: "Chiffres", get: () => afficherChiffres, set: v => afficherChiffres = v },
    { id: "mot", label: "Mot", get: () => afficherMot, set: v => afficherMot = v },
  ];
  options.forEach(opt => {
    const btn = document.createElement("button");
    btn.type = "button";
    btn.className = "param-btn" + (opt.get() ? " active" : "");
    btn.textContent = opt.label;
    btn.onclick = () => {
      // Bascules indépendantes (non exclusives), mais on interdit de
      // désactiver les deux à la fois : il faut toujours voir au moins un
      // format de la cible.
      if (opt.get() && !options.some(o => o !== opt && o.get())) return;
      opt.set(!opt.get());
      btn.classList.toggle("active", opt.get());
      if (game && phase !== "fin") rendreConsigne();
    };
    conteneur.appendChild(btn);
  });
  return conteneur;
...
// Bascule EXCLUSIVE (un seul mode de réponse actif à la fois), contrairement
// aux bascules d'affichage ci-dessus qui sont indépendantes.
...
// Permet de choisir soi-même le nombre cible (utile en classe : cibler un
// nombre lié à la leçon du jour) plutôt que d'attendre un tirage aléatoire.
// Le nombre de jetons requis pour la validation est dérivé de CE nombre
// (game.jetons.length, voir validerExpression/expliquerStructureInvalide),
// pas du mode sélectionné à côté : un nombre choisi à la main n'a donc pas
// besoin de correspondre à l'un des modes 2/3/4 opérations.
function choisirNombrePersonnalise(valeurBrute) {
  const n = parseInt(String(valeurBrute).trim(), 10);
  if (!Number.isInteger(n) || n <= 0) {
    definirMessage("Choisis un nombre entier positif.", "erreur");
    return;
  }
  const d = decomposerNombre(n);
  if (!d || !d.mot) {
    definirMessage("Ce nombre n'a pas d'écriture en toutes lettres reconnue par l'appli.", "erreur");
    return;
  }
  nouvelleQuestion(d);
}

function construireChoixNombre() {
  const conteneur = document.createElement("div");
  conteneur.className = "choix-nombre";

  const input = document.createElement("input");
  input.type = "number";
  input.min = "1";
  input.step = "1";
  input.className = "input-nombre-perso";
  input.placeholder = "Ex : 4271";
  input.setAttribute("aria-label", "Nombre cible à choisir");
  input.addEventListener("keydown", e => {
    if (e.key !== "Enter") return;
    e.preventDefault();
    // Empêche l'Entrée de remonter jusqu'au raccourci clavier global (qui
    // appelle valider()) — ici on veut UNIQUEMENT choisir le nombre.
    e.stopPropagation();
    choisirNombrePersonnalise(input.value);
  });

  const btn = document.createElement("button");
  btn.type = "button";
  btn.className = "panel-btn accent";
  btn.textContent = "Utiliser ce nombre";
  btn.onclick = () => choisirNombrePersonnalise(input.value);

  conteneur.append(input, btn);
  return conteneur;
}
...
function renderPanneauLateral() {
  ...
  ajouterFilet();
  ajouterGroupe("Choisir le nombre", construireChoixNombre());
  ajouterFilet();
  ajouterGroupe("Affichage de la cible", construireBoutonsAffichage());
  ajouterFilet();
...
function ouvrirFiche() {
  ...
  if (sousTitre) sousTitre.textContent = `Mode : ${MODES[modeActuel].label} (${MODES[modeActuel].jetons} jetons par nombre)`;
...
function genererLatexFiche(exercices) {
  ...
  const lignes = exercices.map((ex, i) => {
    ...
    // (\Centerstack, utilisé avant, n'était même pas chargé dans le
    // préambule : un centrage déjà fourni par la colonne suffit.)
...
\\vspace{30pt}
\\begin{center}
  {\\Huge\\bfseries\\color{ardoise} Le compte est bon — NombreMot}\\\\[20pt]
...
Mode~: ${MODES[modeActuel].label} (${nbJetons} jetons par nombre).
}
\\vspace{10pt}
\\begin{center}
...
function telechargerTex() {
  ...
  a.href = url;
  a.download = "fiche-nombremot-compte-est-bon.tex";
