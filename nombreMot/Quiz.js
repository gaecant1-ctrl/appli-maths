...
start() {
  ...
  this.enonceButton.style.display = 'none'; // ⚡ caché au lancement
...
_buildEnonceOverlay() {
  ...
  closeBtn.addEventListener('click', () => {
    this.enonceOverlay.classList.add('hidden');
  });

  this.enonceContent = document.createElement('div');
  this.enonceContent.className = 'enonce-content';

  inner.appendChild(closeBtn);
  inner.appendChild(this.enonceContent);
...
_relireEnonce() {
  ...
  _buildUi() {
    ...
    this.header.appendChild(this.center);
  ...
  /** Création paresseuse du slot niveau */
  getLevelSlot() {
    if (!this.levelSlot) {
      this.levelSlot = document.createElement('div');
      this.levelSlot.id = 'niveau-slot';
      // ⚡ on insère AVANT le bouton Énoncé dans le header center
      this.center.insertBefore(this.levelSlot, this.enonceButton);
    }
    return this.levelSlot;
  }
...
_showReadyScreen() {
  ...
  // ⚡ contenu principal (HTML) passé via texts.readyImage ou texts.readyContent
  if (this.texts.readyContent) {
  ...
  // Texte d’accompagnement (juste en-dessous)
...
_onNextClick() {
  if (!this.hasStarted) {
    ...
    // ⚡ Et seulement maintenant, le bouton Énoncé devient utile
   console.log("Avant toggle:", this.enonceButton.style.display, getComputedStyle(this.enonceButton).display);
this.enonceButton.style.display = '';
console.log("Après toggle:", this.enonceButton.style.display, getComputedStyle(this.enonceButton).display);
