...
class ExerciceExpression extends Exercice {
  ...
  _genererQuestion() {
    ...
    // Affiche seulement si une expressionInitiale est fournie
    if (this.expressionInitiale &&
        (this.questionData?.options?.affichage?.initial?.expression === true ||
  ...
  _reindexPrefixes() {
    let base = 0;
    if (this.initialWrapper) { this.initialWrapper.index = 0; base = 1; }
  ...
  // correction + suite : inchangé, idem ta base
_correction() {
  ...
  _shouldContinue(verdictStatus) {
    ...
    const contFmt = (typeof s.continuerSiFormatIncorrect === 'boolean')
      ? s.continuerSiFormatIncorrect : true;
