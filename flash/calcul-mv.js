/* ================================================================
   ...
   Port depuis evaluation/calcul-mv.js — logique inchangée, uniquement
   adapté en module ES (import/export) pour le projet flash.
...
export function estProduit(noeud) {
...
export function facteursSommes(noeud) {
  if (!noeud) return [];
...
export function reduireConstantes(noeud) {
...
export function evaluerAvecControle(noeud, valeurs) {
  if (noeud instanceof _AtomeMV) {
    const mono = noeud._poly.monomes[0];
...
export function evaluerPolynomeAvecControle(poly, valeurs) {
  let total = ZERO;
