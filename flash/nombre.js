/* ================================================================
   ...
   Port depuis evaluation/Nombre.js — logique inchangée, uniquement adapté
   en module ES (export) pour le projet flash.
================================================================ */
...
toString(opts={}) {
  ...
  toLatex(opts = {}) {
    ...
    // \tfrac (style texte, compact) et non \frac : dans un bloc \[...\]
    // (mode display), \frac s'affiche en grand par défaut — bien trop
    // haut pour une ligne de hauteur fixe (--row-height, jamais modifiée).
    if(r.type==='frac')
        return `\\dfrac{${cl(r.n)}}{${cl(r.d)}}`;

    if(r.type==='mixte')
        return `${cl(r.ent)}+\\dfrac{${cl(r.n)}}{${cl(r.d)}}`;
}
