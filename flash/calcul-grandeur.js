/* ================================================================
   calcul-grandeur.js — grandeur physique (valeur exacte + unité)

   Port depuis appli-maths/nombreProportionnalite/Grandeur.js — logique
   inchangée, uniquement adapté en module ES (import/export) pour le
   projet flash. Dépend de nombre.js (chargé via import).
================================================================ */
...
export class UniteInvalideError extends Error {
...
export class Grandeur {
  ...
  add(other) {
    ...
    return new Grandeur(somme.div(r1.facteur).simplify(), this.uniteDict);
  }
  ...
  /** Élève la grandeur à la puissance entière n (n peut être négatif). L'exposant de chaque unité est multiplié par n. */
  pow(n) {
    const newValeur = this.valeur.pow(n);
    const newUnite = {};
    for (const [u, exp] of Object.entries(this.uniteDict)) {
      const e = exp * n;
      if (e !== 0) newUnite[u] = e;
    }
    return new Grandeur(newValeur, newUnite).nettoyer();
...
// Dans Grandeur
  ...
  // À ajouter dans la classe Grandeur
nettoyer() {
    // 1. On passe tout en unités SI (m, s, kg...)
    // toFondamentales() utilise déjà reduireUniteDict qui fait le gros du travail
    const { valeur, unite } = this.toFondamentales();

    // 2. On essaie de trouver si cette unité "fondamentale" correspond à une nature connue
    const nature = Grandeur.reconnaitreNature(unite);

    if (nature !== "Inconnue" && nature !== "Scalaire") {
        // 3. On convertit vers l'unité de référence de cette nature (ex: m)
        const uniteCible = Grandeur.profilsPhysiques[nature];
        return this.convertirEn(uniteCible);
    }

    // Si c'est une unité complexe (ex: m/s²), on renvoie l'objet réduit au maximum
...
toString(opts = {}) {
  ...
  // Suppression de l'espace entre la valeur et l'unité
...
return uniteLatex ? `${valLatex}\\,${uniteLatex}` : valLatex;
...
export function afficheUnitesCollegien(uniteDict) {
  const num = [];
  const den = [];
...
export function uniteDictToLatexFraction(uniteDict) {
  const num = [];
  const den = [];

  // L'exposant est lui aussi enveloppé dans \text{} : sinon il reste rendu
  // avec la police interne de MathJax alors que l'unité juste avant a été
  // substituée dans la police de la page (mtextInheritFont) — ce mélange
  // de polices donne une impression de décalage/espace, même si les
  // boîtes sont géométriquement jointives.
  for (const [u, exp] of Object.entries(uniteDict)) {
    const base = `\\text{${u}}`;
    if (exp > 0) {
      num.push(exp === 1 ? base : `${base}^{\\text{${exp}}}`);
    } else if (exp < 0) {
      const e = Math.abs(exp);
      den.push(e === 1 ? base : `${base}^{\\text{${e}}}`);
    }
  }
...
export function uniteDictToLatexInline(uniteDict = {}) {
  const num = [];
  const den = [];

  // Même raison que dans uniteDictToLatexFraction : l'exposant doit lui
  // aussi passer par \text{} pour suivre la police substituée de l'unité.
  for (const [u, exp] of Object.entries(uniteDict)) {
    const unit = `\\text{${u}}`;
    if (exp > 0) {
      num.push(exp === 1 ? unit : `${unit}^{\\text{${exp}}}`);
    } else if (exp < 0) {
      const absExp = Math.abs(exp);
      den.push(absExp === 1 ? unit : `${unit}^{\\text{${absExp}}}`);
    }
  }
