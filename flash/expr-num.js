/* ================================================================
   expr-num.js — Évaluation d'expressions numériques exactes (Nombre)
   Port de calcul-prioritaire/calcul-num.js (référence, non modifiée) :
   même arbre d'expression, même arithmétique exacte (Nombre, fractions
   a/b en interne pour les décimaux), mais SANS l'opérateur fraction "/" —
   flash n'utilise que des entiers relatifs et des décimaux, jamais de
   fraction affichée. Seul ":" (division classique) subsiste, au même
   niveau de priorité que "*" (évalués ensemble, de gauche à droite).
...
export class ObjetStringNum {
  ...
  _construireArbre() {
    ...
    // NIVEAU 4 : Les Atomes et Parenthèses (Priorité maximale)
    const parseAtome = () => {
    ...
    // NIVEAU 3 : Les Puissances (^) et Signes Unaires (-, +)
    const parsePow = () => {
      if (peek() === '-') {
        idx++;
    ...
    // NIVEAU 2 : Multiplications (* et implicites) et Divisions (:), même
    // niveau, évaluées de gauche à droite — pas de priorité "fraction"
    // séparée puisque flash n'affiche jamais de fraction a/b.
    const parseMulDiv = () => {
      let left = parsePow();
      while (idx < tokens.length) {
        ...
        if (t === '*') {
          idx++;
          left = new _ProduitNum([left, parsePow()], { implicite: false });
        } else if (t === ':') {
          idx++;
          left = new _QuotientNum([left, parsePow()]);
        } else if (estDebut()) {
          left = new _ProduitNum([left, parsePow()], { implicite: true });
        } else break;
      }
      return left;
    };
...
// Exposé pour evaluerAvecControle (gabarits-num.js), qui a besoin de
// distinguer les types de nœuds sans réimporter la classe interne.
export const _classesArbre = { _AtomeNum, _SommeNum, _DifferenceNum, _ProduitNum, _QuotientNum, _PuissanceNum };
export { ZERO, UN };

// ── Fonctions globales de confort ──────────────────────────────

export function evalNum(text) {
  try {
