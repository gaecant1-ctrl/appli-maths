...
class FichePapier {
  ...
  /**
   * Une entrée par exercice : lettre (A, B, C… cycle via _letterByIndex),
   * expression rendue en LaTeX, et diviseur — format compact "A = ... par b"
   * affiché en grille, sans énoncé long ni numéro (voir _rendreTableau).
   */
  _genererSerie(seed) {
    const liste = [];
    for (let i = 1; i <= this.nbExercices; i++) {
      ...
      try {
        const obj = new ObjetString(variant.exprStr, enonce.modeCorrectionBase);
        latex = obj.arbre.toLatex(enonce.affichageInitial);
      ...
      liste.push({ index: i, lettre, latex, b: variant.b });
    }
    return liste;
  }
  ...
  _construireOverlay() {
    ...
    identite.innerHTML = `
      ...
      <span>Note : <span class="trait court"></span> / 10</span>
    `;
  ...
  _rendreTableau(liste) {
    ...
    liste.forEach(item => {
      ...
      // "par", diviseur) — pas de mélange police HTML / police MathJax en plein
      // milieu de la ligne (qui donnait un rendu disparate).
      cellule.innerHTML = `
        <div class="fiche-enonce">$${item.lettre} = ${item.latex} \\quad \\text{par } ${item.b}$</div>
  ...
  _genererLatex() {
    ...
    const celluleTex = (item) => item
      ? `\\celluleFiche{${item.lettre} = $${item.latex}$ \\quad par $${item.b}$}`
      : '\\celluleFiche{}';
...
\\noindent Nom et prénom : \\hrulefill \\hspace{1cm} Note : \\hrulefill\\,/\\,10
