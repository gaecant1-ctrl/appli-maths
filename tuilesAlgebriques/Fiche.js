/* ==============================================
   Fiche.js — overlay "Fiche papier" pour le quiz tuiles algébriques
   ----------------------------------------------
   Génère une fiche imprimable de 8 expressions à développer et réduire
   (grille 4 lignes x 2 colonnes), chaque cellule contenant l'énoncé en haut
   et un espace vide dessous pour que l'élève écrive sa résolution.

   Les 8 expressions sont tirées avec tirerUnExercice() + construireExprEtSolution()
   (définies dans app.js) — mêmes types actifs (distributivité / produit) et même
   réglage de soustraction (avec/sans) que dans l'atelier/quiz en cours, sans jamais
   toucher à la partie affichée à l'écran.
...
class FichePapier {
  constructor(opts = {}) {
    this.nbLignes = 5;
    this.nbColonnes = 2;
    this.nbExercices = this.nbLignes * this.nbColonnes; // 10, fixe par construction de la grille
    this.titre = opts.titre || "Fiche d'exercices — Développement (tuiles algébriques)";
    this.sousTitre = opts.sousTitre || "";

    this.overlay = null;
    this.grilleWrap = null;
    this._derniereSerie = null; // [{index, expr}, ...] de la série actuellement affichée
  ...
  _genererSerie() {
    // Réutilise tirerUnExercice() + construireExprEtSolution() (app.js) telles quelles —
    // mêmes types actifs et même réglage de soustraction que le quiz en cours. Si jamais
    // absentes (page sans le quiz), la fiche reste vide.
    if (typeof tirerUnExercice !== 'function' || typeof construireExprEtSolution !== 'function') return [];

    const liste = [];
    for (let i = 0; i < this.nbExercices; i++) {
      const d = tirerUnExercice();
      const { expr } = construireExprEtSolution(d);
      liste.push({ index: i + 1, expr });
    }
    return liste;
  }
  ...
  ouvrir() {
    // Toujours régénérer à l'ouverture : les réglages (types actifs, soustraction) ont pu
  ...
  _genererLatex() {
    ...
    // occupent bien la page A4 disponible sans déborder.
    const HAUTEUR_CELLULE_CM = 3.9;
    const ESPACE_HAUT_CM = 0.4; // évite que l'énoncé soit collé au bord supérieur de la cellule

    const cellulesTex = liste.map(item => {
      const enonceTex = `$${this._texEscape(item.expr)}$`;
...
`;
  ...
  _telechargerLatex() {
    ...
    a.href = url;
    a.download = 'fiche-tuiles-developpement.tex';
  ...
  _installerCSS() {
    ...
    style.textContent = `
      ...
      .fiche-actions button:hover{ background:var(--craie-hover, #145ea8); }
