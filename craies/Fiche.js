/* ==============================================
   Fiche.js — overlay "Fiche papier" pour le quiz de réduction d'expressions
   ----------------------------------------------
   Génère une fiche imprimable de 10 expressions à réduire (grille 5 lignes
   x 2 colonnes), chaque cellule contenant l'énoncé en haut et un espace
   vide dessous pour que l'élève écrive sa résolution.

   Les 10 expressions sont tirées avec tirerUnExercice() + construireEnonceLatex()
   (définies dans app.js) — mêmes types actifs (Simple / Multiples) que dans le
   quiz en cours, sans jamais toucher à la partie affichée à l'écran.
...
class FichePapier {
  constructor(opts = {}) {
    ...
    this.titre = opts.titre || "Fiche d'exercices — Réduction d'expressions";
  ...
  /* ---------------- Génération des 10 expressions (sans DOM) ---------------- */

  _genererSerie() {
    // Réutilise tirerUnExercice() + construireEnonceLatex() (app.js) telles quelles — mêmes
    // types actifs que le quiz en cours. Si jamais absentes (page sans le quiz), fiche vide.
    if (typeof tirerUnExercice !== 'function' || typeof construireEnonceLatex !== 'function') return [];

    const liste = [];
    for (let i = 0; i < this.nbExercices; i++) {
      const data = tirerUnExercice();
      liste.push({ index: i + 1, expr: construireEnonceLatex(data) });
    }
    return liste;
  }
  ...
  ouvrir() {
    // Toujours régénérer à l'ouverture : les réglages (types actifs) ont pu changer depuis
    // la dernière série générée.
    this._regenerer();
  ...
  _genererLatex() {
    ...
    // Hauteur de chaque cellule, choisie pour que les 5 lignes (10 expressions)
...
`;
  ...
  _telechargerLatex() {
    ...
    a.href = url;
    a.download = 'fiche-reduction.tex';
  ...
  _installerCSS() {
    ...
    style.textContent = `
      ...
      @media print{
        ...
        .fiche-grille{
          height: 22cm; /* force les 5 lignes à se répartir sur la hauteur restante d'une page A4 */
        }
        .fiche-cellule{
          min-height: 0;
        }
