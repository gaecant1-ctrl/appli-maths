...
case 12: { 
    ...
    sit = `On paye $${p1.toLatex({nombreAff :"dec" })}$ pour $${v1.toLatex({nombreAff :"mixteSimple"})}$ d'un produit.`;
...
case 17: { // Proportionnalité inverse (Vaches et Foin)
   ...
   variantOptions = {
    ...
    quest = `Pendant combien de temps $${v2.toLatex(affOpts)}$ vaches peuvent-elles se nourrir avec cette même réserve ?`;
...
case 21: {
    ...
    // Groupe A = le groupe associé directement au pctBase
    const nomA = this.rng.pick(['garçons', 'filles']);
    const pctA = pctBase;
    const valA = (nomA === 'garçons') ? a * k : tot - (a * k);

    // Groupe B = le groupe complémentaire (100 - pctBase)
...
case 24: {
    ...
    // On veut que ce soit supérieur à persOrigine pour l'agrandissement, donc on applique un facteur libre
    const facteurAgrandissement = this.rng.pick([2, 3, 4]);
    const persMaxPossible = persOrigine * facteurAgrandissement; 

    // 4. Ingrédient 1 (Le limiteur - tombe pile sur persMaxPossible)
    const coefIngr1 = this.rng.pick([30, 40, 50, 60, 100]);
    const qteOrigine1 = coefIngr1 * persOrigine;
    const reserve1 = coefIngr1 * persMaxPossible;

    // 5. Ingrédient 2 (Celui en surplus)
    const coefIngr2 = this.rng.pick([20, 25, 75, 80]);
    const qteOrigine2 = coefIngr2 * persOrigine;
    const surplusPersonnes = this.rng.pick([1, 2, 3]) * persOrigine; // Surplus net et propre
