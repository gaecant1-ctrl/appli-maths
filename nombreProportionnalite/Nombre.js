...
if (/^[+-]?\d*(?:[.,]\d+)?\s*%$/.test(s)) {
    ...
    // On stocke sous forme de fraction n/100
    // Si c'est un décimal (ex: 12.5%), on ajuste pour avoir des entiers (125/1000)
    const parts = valeurStr.split('.');
    const puissanceDix = parts[1] ? Math.pow(10, parts[1].length) : 1;
    
    return { 
        a: Math.round(num * puissanceDix), 
        b: 100 * puissanceDix 
    };
}
...
isFormat(type) {
  ...
  _getRendu(mode = 'brut', precision = 2) {
    ...
    if (this.isEntier() && mode !== 'brut') {
      return { type: 'simple', v: String(a / b) };
    }

    switch (mode) {
      case 'dec':
        if (this.isDecimal()) return { type: 'simple', v: this.getValDec(10) };
        return this._getRendu('fractionSimple');

      case 'fraction':
        return { type: 'frac', n: a, d: b };

      case 'fractionSimple':
        const d = gcd(a, b);
        return { type: 'frac', n: a / d, d: b / d };

      case 'mixte':
      case 'mixteSimple':
        const ent = a >= 0 ? Math.floor(a / b) : -Math.ceil(Math.abs(a) / b);
        let n = Math.abs(a - ent * b), den = b;
        if (ent === 0) return this._getRendu(mode === 'mixteSimple' ? 'fractionSimple' : 'fraction');
        if (mode === 'mixteSimple') { const c = gcd(n, den); n /= c; den /= c; }
        return { type: 'mixte', ent, n, d: den };

      case 'decApprox':
        if (this.isDecimal()) return { type: 'simple', v: this.getValDec(10) };
        return { type: 'simple', v: this.getValDec(precision) };
      ...
      case 'brut':
      default:
        return { type: 'brut', v: this.initial };
    }
  }
  ...
  _getTypeHerite(autre) {
    ...
    // En cas de conflit (ex: mixte + dec), la fraction est le refuge sécurisé
    return 'fraction';
  }
  ...
  // ———————————————————
  toString(opts = {}) {
    const r = this._getRendu(opts.nombreAff, opts.precision);
    if (r.type === 'simple' || r.type === 'brut') return r.v;
    if (r.type === 'frac') return `${r.n}/${r.d}`;
    if (r.type === 'mixte') return `${r.ent}+${r.n}/${r.d}`;
  }

  toLatex(opts = {}) {
    const r = this._getRendu(opts.nombreAff, opts.precision);
    // Règle décidée : On conserve le point décimal (suppression du replace point par virgule)
    const cl = (v) => String(v).replace(/\s+/g, '');
    ...
    if (r.type === 'brut') {
      ...
      // Cas spécifique pour le mixte brut "1+2/3"
      if (v.includes('+') && v.includes('/')) {
        const [entier, fraction] = v.split('+');
        const [n, d] = fraction.split('/');
        return `${cl(entier)}+\\frac{${cl(n)}}{${cl(d)}}`;
      }
      
      // Cas fraction brute "10/20"
      if (v.includes('/')) {
        const p = v.split('/');
        return `\\frac{${cl(p[0])}}{${cl(p[1])}}`;
      }
