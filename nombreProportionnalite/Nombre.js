...
getValDec(precisionMax = 10, arrondi = false) {
  // 1. On travaille exclusivement avec les entiers stockés dans valeurNum
  ...
  // Partie entière
  const entierAbs = Math.floor(num / den);
  let reste = num % den;

  if (reste === 0) return (sign * entierAbs).toString();

  let decimals = "";
  // 2. Division longue "à la main" sur des entiers
  for (let i = 0; i < precisionMax && reste !== 0; i++) {
    reste *= 10;
    const chiffre = Math.floor(reste / den);
    decimals += String(chiffre);
    reste = reste % den;
  }

  // 3. Suppression des zéros de fin (uniquement ceux issus de la division)
...
toString(opts = {}) {
  ...
  if (nombreAff === "auto") {
    if (b === 1) return `${a}`; // Entier
    if (this.isDecimal()) return this.getValDec(precision, arrondi); // Décimal
    // Sinon fraction simplifiée
...
toLatex(opts = {}) {
  console.log("objet",this,opts);
  const { a, b } = this.valeurNum;
  const { nombreAff = "auto", precision = 10, arrondi = false } = opts;
  

   console.log("nombreAff",nombreAff);
  const formatLat = (n) => n.toString().replace(/\B(?=(\d{3})+(?!\d))/g, "\\,");

  if (nombreAff === "auto") {
    if (b === 1) return formatLat(a);
    if (this.isDecimal()) return this.getValDec(precision, arrondi);
    
    const simp = this.simplify().valeurNum;
    return simp.b === 1 ? formatLat(simp.a) : `\\frac{${formatLat(simp.a)}}{${formatLat(simp.b)}}`;
  }
  ...
  if (this.typeEcriture === "dec" || nombreAff === "dec") {
    return this.getValDec(precision, arrondi);
  }

  return  `\\frac{${formatLat(a)}}{${formatLat(b)}}`;
}
