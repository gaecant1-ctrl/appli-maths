class Nombre {
  constructor(s) {
    ...
    // ça, taper "2^3" échouait au parsing ("^" n'est reconnu que comme
    // suffixe d'unité, ex: "m^2") ou affichait "8" au lieu de "2^3".
    // NB : contrairement à decompositionEuclidienne/decompositionProduit,
    // le Grader.js de cette appli n'a pas de mécanisme memesAtomes/
    // memesOperations basé sur les tokens (ses policies "memesAtomes"/
    // "memesOperations" sont à false par défaut) — donc pas de développement
    // en multiplication répétée à faire ici pour l'équivalence de forme.
    const mPuissance = /^([+-]?\d+(?:\.\d+)?)\^(\d+)$/.exec(this.initial);
    if (mPuissance) {
      ...
      this.valeurNum = { a: Math.round(Math.pow(parseFloat(base), exposant)), b: 1 };
      return;
    }
