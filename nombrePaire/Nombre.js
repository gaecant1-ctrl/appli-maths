/* ================================================================
   Nombre.js — nombre exact (fraction a/b), autonome (aucune dépendance)
   ----------------------------------------------
   Extrait de calcul-litteral.js pour être réutilisable partout : c'est la
   dépendance commune de calcul-mv.js (coefficients fractionnaires) et de
   calcul-litteral.js (Polynome à une variable), qui doivent tous deux être
   chargés APRÈS ce fichier.
================================================================ */
...
case 'pourcentage': {
  // Évite la division flottante (a/b)*100, qui produit des artefacts
  // d'arrondi binaire (ex: 9.9999999997 au lieu de 10) pour des fractions
  // non représentables exactement en binaire. On réutilise le même calcul
  // entier exact que getValDec, appliqué à (a*100)/b.
  const pourcent = Nombre.fromParts(this.valeurNum.a * 100, this.valeurNum.b);
  return {
    type: 'simple',
    v: this._formaterMilliers(pourcent.getValDec(10)) + '%'
  };
}
...
add(o) {
    const b1 = this.valeurNum.b, b2 = o.valeurNum.b;
    const b = lcm(b1, b2);
    const a = this.valeurNum.a * (b / b1) + o.valeurNum.a * (b / b2);
  ...
  /** Oppose (change de signe), sans passer par une soustraction à 0. */
  neg() {
    return Nombre.fromParts(-this.valeurNum.a, this.valeurNum.b, this.typeEcriture);
  }

  /** Valeur absolue. */
  abs() {
    return this.valeurNum.a < 0 ? this.neg() : this;
  }
