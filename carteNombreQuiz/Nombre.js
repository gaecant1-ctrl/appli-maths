...
if (format === "forme") {
return Array.isArray(this.forme)
? this.forme.join("-")
: this.forme
}
...
if (format === "fractionSimp") {

const { a:sa, b:sb } = this.simplify().valeurNum
...
if(arrondi === "round") x = Math.round(x*f)/f
if(arrondi === "floor") x = Math.floor(x*f)/f
if(arrondi === "ceil") x = Math.ceil(x*f)/f
if(arrondi === "trunc") x = Math.trunc(x*f)/f
...
if (format === "fractionDecimal") {

const dec = this.toString("decimal",{precision,arrondi})

if(!dec.includes(".")) return `${dec}/1`
...
if(num === 0) return `${entier}`

return `${entier}+${num}/${denom}`
...
const entier = Math.trunc(sa / sb)

const reste = Math.abs(sa % sb)

if(reste === 0) return `${entier}`

return `${entier}+${reste}/${sb}`

}


/* =========================
décomposition premiers
========================= */

if (format === "decompPrem") {

const facteurs = this.decompFacteursPremiers()

let parts = []

for (let f in facteurs) {

for (let i = 0; i < facteurs[f]; i++) {
parts.push(f)
}
...
if (format === "decompEuclid") {

const { quotient:q, reste:r } =
this.divisionEuclidienne(base)

if(r === 0 && !forceResteZero){
return `${q}*${base}`
}
...
if(this.isDecimal()){
return this.toString("decimal",{precision,arrondi})
}
...
if(!dec.includes(".")) return `${FRAC}{${dec}}{1}`
...
if(this.isDecimal()){
return this.toLatex("decimal",{precision,arrondi})
}
