...
// Opérateurs
    if (isOp(c)) {
      ...
      // On ne fusionne le signe avec le nombre QUE pour le moins unaire (-)
      // On laisse le plus (+) toujours binaire pour éviter de casser les sommes comme (9+4.5)
      if (c === '-' && unaryContext) {
        let j = i + 1;
      ...
      // Par défaut (et toujours pour le +), on le pousse comme opérateur séparé
      out.push(c);
      prev = 'op';
      i++;
      continue;
...
if (erreurs.length > 0) {
    const e = new Error("PARSE_ERROR"); // On simplifie le message interne
    e.code = "PARSE_ERROR";
    throw e;
}
