...
toString(opts={}) {
  ...
  toLatex(opts = {}) {
    ...
    if(r.type==='simple'){
        // Convention française : virgule décimale plutôt que point, mais
        // uniquement pour les modes réellement décimaux (dec/decApprox) —
        // un entier ou un pourcentage n'a jamais de point à convertir, donc
        // ce remplacement ciblé ne risque pas d'altérer d'autres formats.
        const txt = cl(r.v);
        return (opts.nombreAff === 'dec' || opts.nombreAff === 'decApprox')
          ? txt.replace('.', ',')
          : txt;
    }
