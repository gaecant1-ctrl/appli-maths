...
/* Capacité : unités de contenance (L, dL, cL, mL, daL, hL, kL) ET unités de
   volume (mm3 → km3), toutes ramenées à une base commune en litres, pour
   passer d'une famille à l'autre (ex : 1 dm3 = 1 L, 1 cm3 = 1 mL, 1 m3 = 1 kL). */
class Capacite {
  static facteurs = {
    mm3: -6,
    cm3: -3, mL: -3,
  ...
  constructor(texte) {
    const match = texte.trim().match(/^([\d.,]+)\s*(km3|hm3|dam3|mm3|cm3|dm3|m3|kL|hL|daL|dL|cL|mL|L)$/i);
    if (!match) throw new Error("Format invalide");

    this.valeur = match[1].replace(',', '.');
    this.uniteInitiale = match[2];
    this.litres = diviserParPuissanceDe10(this.valeur, -1 * Capacite.facteurs[this.uniteInitiale]);
  }

  en(unite) {
    if (!(unite in Capacite.facteurs)) throw new Error("Unité inconnue : " + unite);
    const puissance = Capacite.facteurs[unite];
    return diviserParPuissanceDe10(this.litres, puissance);
  }

  estEgal(autre) {
    return Math.abs(this.litres - autre.litres) < 1e-9;
  }

  latex(unite = this.uniteInitiale) {
    const valeur = this.en(unite);
    if (/3$/.test(unite)) {
      const uniteSansExposant = unite.replace(/3$/, '');
      return `${valeur}\\ \\mathrm{${uniteSansExposant}}^3`;
    }
    return `${valeur}\\ \\mathrm{${unite}}`;
  }
...
/* ---------------------------------------------------------
   ...
   typesAutorises : sous-ensemble de ['longueur','aire','volume','capacite'].
   Si vide ou absent, les quatre types sont utilisés (aléatoire).
   Retourne { typeExercice, unite1, unite2, question, bonneReponse, uniteCible }
--------------------------------------------------------- */
/* Capacité : toujours une unité de contenance (L, dL, cL, mL, daL, hL, kL)
   d'un côté et une unité de volume (mm3 → km3) de l'autre — jamais deux
   unités de la même famille — pour bien travailler le passage litres ⇄ cube. */
function genererExerciceCapacite() {
  const litreUnites = ['mL', 'cL', 'dL', 'L', 'daL', 'hL', 'kL'];
  const cubeUnites = ['mm3', 'cm3', 'dm3', 'm3', 'dam3', 'hm3', 'km3'];

  let uniteLitre, uniteCube;
  do {
    uniteLitre = litreUnites[Math.floor(Math.random() * litreUnites.length)];
    uniteCube = cubeUnites[Math.floor(Math.random() * cubeUnites.length)];
  } while (Math.abs(Capacite.facteurs[uniteLitre] - Capacite.facteurs[uniteCube]) > 6);

  // Choisit aléatoirement lequel des deux est affiché (unite1) et lequel est la cible (unite2)
  const [unite1, unite2] = Math.random() < 0.5 ? [uniteLitre, uniteCube] : [uniteCube, uniteLitre];

  const ecart = Math.abs(Capacite.facteurs[unite1] - Capacite.facteurs[unite2]);
  const valeur = nombreSignificatifAmeliore('capacite', 0, ecart);

  const bonneReponse = new Capacite(`${valeur} ${unite2}`);
  const question = new Capacite(`${bonneReponse.en(unite1)} ${unite1}`);

  return { typeExercice: 'capacite', unite1, unite2, question, bonneReponse, uniteCible: unite2 };
}
...
function genererExerciceConversion(typesAutorises) {
  const typesDisponibles = (Array.isArray(typesAutorises) && typesAutorises.length > 0)
    ? typesAutorises
    : ['longueur', 'aire', 'volume', 'capacite'];

  const typeExercice = typesDisponibles[Math.floor(Math.random() * typesDisponibles.length)];

  if (typeExercice === 'capacite') return genererExerciceCapacite();
