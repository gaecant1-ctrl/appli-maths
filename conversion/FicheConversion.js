...
class FicheConversion {
  constructor(opts = {}) {
    this.nbExercices = opts.nbExercices || 10;
    this.titre = opts.titre || "Fiche d'exercices — Conversions";
    this.sousTitre = opts.sousTitre || "Longueur, aire, volume, capacité";
  ...
  _enonceLatex(item) {
    ...
    else if (item.typeExercice === 'capacite' && /3$/.test(item.unite2)) {
      exposant = `\\mathrm{${item.unite2.slice(0, -1)}}^3`;
