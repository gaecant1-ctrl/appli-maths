/* ==============================================
   Fiche.js — overlay "Fiche papier" pour le quiz d'évaluation numérique
   ----------------------------------------------
   Génère une fiche imprimable de 8 expressions à évaluer pour des valeurs
   données des lettres (grille 4 lignes x 2 colonnes), chaque cellule
   contenant l'énoncé + les valeurs en haut et un espace vide dessous pour
   que l'élève écrive son calcul.

   Les 8 exercices sont générés avec generateRandomExpression() (définie
   dans app.js) — même banque de types, même niveau et nombre de lettres
   que dans le quiz — sans jamais toucher à la partie en cours.
...
class FichePapier {
  constructor(opts = {}) {
    ...
    this.titre = opts.titre || "Fiche d'exercices — Évaluation numérique";
    this.sousTitre = opts.sousTitre || "Calcule chaque expression pour les valeurs données.";

    this.overlay = null;
    this.grilleWrap = null;
    this._derniereSerie = null; // [{expr, texteValeurs}, ...] de la série actuellement affichée
  ...
  /* ---------------- Génération des 8 expressions (sans DOM) ---------------- */

  _genererSerie() {
    // Réutilise generateRandomExpression() (app.js) telle quelle : même banque
    // de types, mêmes réglages (niveau, nombre de lettres), même filet de
    // sécurité anti-génération invalide. Si jamais absente, la fiche reste vide.
    if (typeof generateRandomExpression !== 'function') return [];

    const liste = [];
    for (let i = 0; i < this.nbExercices; i++) {
      const obj = generateRandomExpression(); // ExerciceEvaluation {expr, valeurs, resultat}
      liste.push({ index: i + 1, exprLatex: obj.enonceLatex(), texteValeursLabel: obj.texteValeursLabel() });
    }
    return liste;
  }
  ...
  _rendreGrille(liste) {
    ...
    liste.forEach(item => {
      ...
      enonce.innerHTML = `
        <div style="text-align:left; font-weight:700; color:#000; margin-bottom:8px;">${item.texteValeursLabel}</div>
        <div style="text-align:center;">\\(${item.exprLatex}\\)</div>
      `;
  ...
  _genererLatex() {
    ...
    // Hauteur de chaque cellule, choisie pour que les 4 lignes (8 expressions)
    // occupent bien la page A4 disponible sans déborder (même valeurs que la
    // fiche équations, dont la disposition en grille est identique).
    ...
    const cellulesTex = liste.map(item => {
      // exprLatex déjà calculé par ExerciceEvaluation.enonceLatex() — même rendu
      // (réduit ou forme écrite) que celui affiché dans l'overlay HTML.
      const exprTex = item.exprLatex;
      // "Pour ..." à gauche, bien visible (gras, en évidence) ; l'expression
      // en dessous, centrée : deux alignements différents dans la même cellule.
      const labelTex = this._texEscape(item.texteValeursLabel);
      const enonceTex = `\\raggedright\\textbf{${labelTex}}\\par\\vspace{6pt}\\centering $${exprTex}$`;
      ...
      return `\\parbox[t][${HAUTEUR_CELLULE_CM}cm][t]{\\largeurcell}{\\vspace{${ESPACE_HAUT_CM}cm}${enonceTex}}`;
    });
...
`;
  ...
  _telechargerLatex() {
    ...
    a.href = url;
    a.download = 'fiche-evaluation.tex';
