/* ================================================================
   ...
   ----------------------------------------------
   Basé sur la version de l'appli "evaluation" (rendu texte/LaTeX plus riche :
   typeEcriture, formes canonique/mixte/pourcentage...), avec en plus les
   fonctions random()/randomInt() utilisées par Configuration.js — nécessaires
   car p5.js tourne ici en mode instance (pas de random() global fourni par p5).
================================================================ */
...
/** random([a,b,c]) → élément au hasard ; random(max) → [0,max[ ; random(min,max) → [min,max[. */
...
_formaterMilliers(v) {
  ...
  _getRendu(mode = 'brut', precision = 2) {
    ...
    // Alias propres à l'appli abscisseFinale : "auto" (meilleure écriture
    // possible) et "fractionMixte" (nombre entier + fraction), qui n'existent
    // pas sous ce nom dans le moteur d'origine.
    if (mode === 'auto') return this._getRendu('canonique', precision);
    if (mode === 'fractionMixte') return this._getRendu('mixteSimple', precision);
