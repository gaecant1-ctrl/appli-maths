/* ==============================================
   Fiche.js — overlay "Fiche papier" pour le quiz développement/réduction
   ----------------------------------------------
   ...
   Les 8 expressions sont générées avec generateRandomExpression() (définie
   dans app.js) — même banque de types, mêmes lettres aléatoires (x, y, z,
   a, b) que dans le quiz — sans jamais toucher à la partie en cours.
...
class FichePapier {
  constructor(opts = {}) {
    ...
    this.titre = opts.titre || "Fiche d'exercices — Développement et réduction";
    this.sousTitre = opts.sousTitre || "";

    this.overlay = null;
    this.grilleWrap = null;
    this._derniereSerie = null; // [{expr, lettre}, ...] de la série actuellement affichée
  ...
  _genererSerie() {
    ...
    // de types, mêmes lettres aléatoires, même filet de sécurité anti-génération
    // invalide. Si jamais absente (page sans le quiz), la fiche reste vide.
    if (typeof generateRandomExpression !== 'function') return [];

    const liste = [];
    for (let i = 0; i < this.nbExercices; i++) {
      const obj = generateRandomExpression(); // ExpressionLitterale {expr, lettre}
      liste.push({ index: i + 1, expr: obj.expr, lettre: obj.lettre });
    }
    return liste;
  }
...
`;
  ...
  _telechargerLatex() {
    ...
    a.href = url;
    a.download = 'fiche-developpement.tex';
  ...
  _installerCSS() {
    ...
    style.textContent = `
      ...
      .fiche-actions button:hover{ background:var(--craie-hover, #3a7bc8); }
