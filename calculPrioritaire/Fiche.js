/* ==============================================
   Fiche.js — overlay "Fiche papier" pour le quiz de calcul prioritaire
   ----------------------------------------------
   Génère une fiche imprimable de 8 expressions purement numériques à calculer
   en respectant les priorités opératoires (grille 4 lignes x 2 colonnes),
   ...
   dans app.js) — même banque de types, même niveau et type de nombres
...
class FichePapier {
  constructor(opts = {}) {
    ...
    this.titre = opts.titre || "Fiche d'exercices — Calcul prioritaire";
    this.sousTitre = opts.sousTitre || "Calcule chaque expression en respectant les priorités opératoires.";

    this.overlay = null;
    this.grilleWrap = null;
    this._derniereSerie = null; // [{expr}, ...] de la série actuellement affichée
  ...
  _genererSerie() {
    ...
    // de types, mêmes réglages (niveau, type de nombres), même filet de
    ...
    for (let i = 0; i < this.nbExercices; i++) {
      const obj = generateRandomExpression(); // ExerciceEvaluation {expr, resultat}
      liste.push({ index: i + 1, exprLatex: obj.enonceLatex() });
    }
    return liste;
  }
  ...
  _genererLatex() {
    ...
    const cellulesTex = liste.map(item => {
      ...
      // (forme écrite, parenthèses gardées) que celui affiché dans l'overlay HTML.
      const exprTex = item.exprLatex;
      const enonceTex = `\\centering $${exprTex}$`;
