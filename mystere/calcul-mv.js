...
_tokeniser(expr) {
    ...
    while (i < s.length) {
      ...
      //
      // Un chiffre collé à une lettre ne doit JAMAIS fusionner dans le même
      // token ("3n" en un seul bloc) : sinon "2/3n" tokenise en ['2',':','3n']
      // et "n" se retrouve piégé dans le dénominateur d'une fraction qui ne
      // le concernait pas ("2/(3n)" au lieu de "(2/3)*n"). On sépare donc la
      // partie numérique (chiffres, virgule/point, %) de la partie lettres :
      // le "3" et le "n" redeviennent deux tokens distincts, recollés par la
      // multiplication implicite du parseur — au bon niveau de précédence,
      // le même que "*", pas plus fort que ":" — et affichés sans "\times"
      // (voir _ProduitMV.toLatex, implicite ⇒ séparateur vide) : "3n" reste
      // "3n" à l'écran, exactement comme avant ce correctif.
      if (isAtomChar(c)) {
        const isNum = ch => /[0-9.,%]/.test(ch);
        const isLettre = ch => /[a-zA-Z]/.test(ch);
        let k = i + 1;
        if (isNum(c)) {
          while (k < s.length && isNum(s[k])) k++;
        } else {
          while (k < s.length && isLettre(s[k])) k++;
        }
