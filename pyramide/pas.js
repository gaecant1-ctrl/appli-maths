...
window.onload = () => {
    ...
    const conteneurActions = document.getElementById("topButtonsBar");
    if (conteneurActions && typeof FichePyramide === 'function') {
        const fiche = new FichePyramide(window.instanceApp);
        fiche.installerBouton(conteneurActions);
    }

    if (conteneurActions) {
        const filet = document.createElement('div');
        filet.className = 'filet-header';
        conteneurActions.appendChild(filet);
    }
