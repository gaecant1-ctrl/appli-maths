...
if (!this.etatsEtoiles) {
  this.etatsEtoiles = [];
}

// on complète seulement
while (this.etatsEtoiles.length < this.niveau) {
  this.etatsEtoiles.push(0);
}
...
ajouterErreur() {
  ...
  const i = this.niveau - 1;

  if (this.etatsEtoiles[i] < 2) {
    this.etatsEtoiles[i]++;
  }

  // 🔥 effet visuel SUR l’étoile
  setTimeout(() => {

    const stars = document.querySelectorAll("#stars .star");

    if (stars[i]) {
      stars[i].classList.add("error");

      setTimeout(() => {
        stars[i].classList.remove("error");
      }, 300);
    }
...
mettreAJourEtoile() {
  ...
  for (let i = 0; i < this.niveau; i++) {
    ...
    const etat = this.etatsEtoiles[i] ?? 0;

    if (etat === 0) star.classList.add("full");
    else if (etat === 1) star.classList.add("mid");
    else star.classList.add("low");
