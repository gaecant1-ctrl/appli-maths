...
this.mettreAJourEtoile();
    ...
    for (let col = 0; col < this.cols; col++) {
      for (let row = 0; row < this.rows; row++) {
        ...
        // durée mixte "h + min" : texte plus long, on réduit la police
        // pour qu'il tienne dans le jeton au lieu de déborder.
        if (objet.type === "duree" && objet.fiche().includes("\\mathrm{h}") && objet.fiche().includes("\\mathrm{min}")) {
