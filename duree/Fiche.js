...
export class FicheAppariement {
  ...
  constructor(getNiveauMax, opts = {}) {
    ...
    this.titre = opts.titre || "Fiche d'exercices — Conversions d'heures";
    this.sousTitre = opts.sousTitre || "Complete la case vide de chaque ligne.";
  ...
  _genererSerie() {
    ...
    const minutesDejaVues = new Set();
    const lignes = [];
    const tentativesMaxParNiveau = 200; // limite avant de considérer un type "épuisé" pour cette ligne

    for (let i = 0; i < this.nbLignes; i++) {
      // Répartition équilibrée entre les types : on cycle 1,2,3,1,2,3,...
      // plutôt que de tirer un niveau au hasard à chaque ligne — sinon les
      // types ayant peu de valeurs uniques possibles (ex: "decimal", limité
      // aux dénominateurs 2/4/5/10) finissent sous-représentés, leurs
      // tentatives échouant plus souvent sur des doublons déjà utilisés.
      const niveauPrefere = (i % niveauMax) + 1;
      const autresNiveaux = Array.from({ length: niveauMax }, (_, k) => k + 1)
        .filter(n => n !== niveauPrefere);
      const ordreNiveaux = [niveauPrefere, ...autresNiveaux];

      let paireValide = null;

      recherche:
      for (const niveau of ordreNiveaux) {
        for (let t = 0; t < tentativesMaxParNiveau; t++) {
          const paire = new Paire(niveau);
          const [fraction, duree] = paire.objets;
          if (!minutesDejaVues.has(duree.totalMinutes)) {
            paireValide = { fraction, duree };
            break recherche;
          }
        }
        // Réservoir de ce type épuisé pour cette ligne : on passe au type
        // suivant dans l'ordre (repli), sans renoncer à la ligne.
      }

      if (!paireValide) break; // plus aucune valeur disponible nulle part : on arrête là

      minutesDejaVues.add(paireValide.duree.totalMinutes);

      // L'écriture décimale n'a d'intérêt que comme donnée de départ à
      // convertir en heures/minutes — jamais comme réponse attendue (on ne
      // demande pas à l'élève de convertir un "h et min" en décimal).
      // Donc : toujours affichée à gauche, jamais celle qu'on masque.
      const montrerGauche = paireValide.fraction.estDecimal ? true : (Math.random() < 0.5);
      const indexLocal = (lignes.length % this.nbParColonne) + 1;

      lignes.push({
        index: indexLocal,
        gauche: montrerGauche ? paireValide.fraction.fiche() : "",
        droite: montrerGauche ? "" : paireValide.duree.fiche()
      });
    }
...
\\multicolumn{1}{c|}{\\bfseries En heures} &
\\multicolumn{1}{c|}{\\bfseries En h. et min.} \\\\
\\hline
${lignesTex}
\\end{tabular}`;
  }
...
`;
  ...
  _telechargerLatex() {
    ...
    a.href = url;
    a.download = 'fiche-conversions-heures.tex';
  ...
  _installerCSS() {
    ...
    style.textContent = `
      ...
      #btnFermerFicheApp:hover{ background:rgba(255,77,109,0.18); color:#ff4d6d; }
