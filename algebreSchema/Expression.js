...
toLatex(opts) {
  const { affichage = null, parentheseObligatoire = null } = opts;
  const valeur = this.grandeur?.valeur;
...
mul(other, opts) {
  ...
  // Cas scalaire × Prix → on veut un Prix (symétrique au cas Duree
  // ci-dessus). Sans ça, "3*6€" (scalaire à gauche) perd le typage Prix :
  // Prix.mul() ne gère que "6€*3" (Prix à gauche), donc "3*6€" retombe
  // ici et ressortait en simple Atome, faisant échouer une soustraction
  // ultérieure ("Soustraction uniquement entre Prix.").
  if (isThisScalaire && other instanceof Prix) return new Prix(produit, opts);
  if (isOtherScalaire && this instanceof Prix) return new Prix(produit, opts);
...
const affichage =
  affichageClasse ?? (this.doitAfficherMixte() ? "mixte" : "standard");
  }
...
toLatex(opts = {}) {
  ...
  // --- 4. LOGIQUE STANDARD ---
  // Si on n'est pas en mode mixte, on laisse le comportement par défaut (120s)
