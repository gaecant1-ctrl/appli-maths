...
parse(s) {
    ...
    if (mixteRegex.test(s)) {
        ...
        // Convention floor (cohérente avec le reste du projet) : la partie
        // entière saisie (ent) porte déjà tout le signe, et num (toujours
        // positif tel qu'écrit) s'additionne SANS ajustement de signe.
        // Ex: "-2+2/9" représente -2 + 2/9 = -16/9 (PAS -20/9). L'ancienne
        // formule "ent*den + signe*num" appliquait à tort le signe de ent à
        // num aussi (revenant à soustraire num quand ent est négatif), ce
        // qui ne correspond pas à la convention "partie entière la plus
        // négative possible, reste toujours positif" utilisée ailleurs.
        const a = ent * den + num;
        return { a, b: den };
    }
...
toString(opts={}) {
    ...
    if(r.type==='frac'){
        // Signe extrait une seule fois et placé devant la barre de
        // fraction, plutôt qu'au numérateur ("-43/7" plutôt que le
        // mathématiquement correct mais peu académique "−43/7" affiché
        // comme numérateur négatif d'une fraction par ailleurs positive).
        const n = cl(r.n);
        const negatif = n.startsWith('-');
        const nAbs = negatif ? n.slice(1) : n;
        return `${negatif ? '-' : ''}${nAbs}/${cl(r.d)}`;
    }
  ...
  toLatex(opts = {}) {
    ...
    if(r.type==='simple'){
        // Convention française : virgule décimale plutôt que point. On
        // détecte le point sur r.v (avant cl(), donc avant que les
        // séparateurs de milliers ¤ ne soient échappés en "\,") plutôt que
        // de se fier à opts.nombreAff : un mode comme 'canonique' ou 'brut'
        // peut très bien se résoudre en une écriture décimale (ex: 15/8,
        // une fraction, est exactement 1,875) sans que opts.nombreAff vaille
        // littéralement 'dec' — la conversion doit suivre le résultat
        // réellement produit, pas le mode demandé.
        const brut = String(r.v);
        const estDecimal = /^-?[\d¤]+\.\d+$/.test(brut);
        return cl(estDecimal ? brut.replace('.', ',') : brut);
    }

    if(r.type==='frac'){
        // Même principe que toString() : signe extrait une seule fois,
        // placé devant \frac plutôt qu'au numérateur.
        const n = cl(r.n);
        const negatif = n.startsWith('-');
        const nAbs = negatif ? n.slice(1) : n;
        return `${negatif ? '-' : ''}\\frac{${nAbs}}{${cl(r.d)}}`;
    }
